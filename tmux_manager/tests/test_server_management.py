...
def test_ensure_tmux_server_is_running_already_running() -> None:
    ...
    """
    with patch(
        "src.tmux_manager.server_management._is_tmux_server_running"
    ) as mock_is_running:
...
def test_ensure_tmux_server_is_running_start_via_systemd() -> None:
    ...
    with patch(
        ...
        "src.tmux_manager.server_management._is_systemd_service_available"
    ) as mock_systemd_available, patch(
        "src.tmux_manager.server_management._start_tmux_via_systemd"
    ) as mock_start_systemd:
        # Configure mocks
...
def test_ensure_tmux_server_is_running_start_directly() -> None:
    ...
    with patch(
        ...
        "src.tmux_manager.server_management._start_tmux_directly"
    ) as mock_start_directly:
        # Configure mocks
        mock_is_running.return_value = False
