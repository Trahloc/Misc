...
SEARCH_DIRS=("${GIT_ROOT}/${PROJECT_SUBDIR}/src" "${GIT_ROOT}/${PROJECT_SUBDIR}/tests" "${GIT_ROOT}/${PROJECT_SUBDIR}/scripts")
...
"${RG_PATH}" --no-heading --line-number --glob '*.py' --regexp '# (TODO:|noqa: E402)' "${SEARCH_DIRS[@]}" > "${TMP_RG_OUT}" || echo "Warning: rg found no relevant comments or failed. Exit code: $?"
