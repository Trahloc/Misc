...
def get_main_and_dev_dependencies_from_toml(toml_path: Path) -> set[str]:
    ...
    try:
        ...
        optional_deps_section_match = re.search(r"^\\[project\\.optional-dependencies\\](.*?)(?:\\n^\\[|$)", content, re.DOTALL | re.MULTILINE | re.IGNORECASE)
        if optional_deps_section_match:
            ...
            dev_deps_match = re.search(r"^dev\\s*=\\s*\\[(.*?)\\]", optional_deps_content, re.DOTALL | re.MULTILINE | re.IGNORECASE)
            if dev_deps_match:
        ...
        if not dependencies: # Only try Poetry if PEP 621 parsing yielded nothing
            ...
            poetry_main_match = re.search(r"\[tool\.poetry\.dependencies\](.*?)(\n\[|$)", content, re.DOTALL | re.IGNORECASE)
            ...
            poetry_dev_match = re.search(r"\[tool\.poetry\.group\.dev\.dependencies\](.*?)(\n\[|$)", content, re.DOTALL | re.IGNORECASE)
            # Alternative Poetry dev group syntax (older)
            if not poetry_dev_match:
                poetry_dev_match = re.search(r"\[tool\.poetry\.dev-dependencies\](.*?)(\n\[|$)", content, re.DOTALL | re.IGNORECASE)
        ...
        if not dependencies:
             print(f"Warning: Could not find or parse dependencies under [project.dependencies], [project.optional-dependencies.dev], or Poetry sections in {toml_path}", file=sys.stderr)
