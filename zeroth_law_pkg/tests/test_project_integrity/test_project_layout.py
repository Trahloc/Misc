...
def _expected_test_path(src_file: Path) -> Path:
    ...
    # Prefix directory parts ONLY if they don't start with '_'
    test_parts = []
    for p in parts[:-1]:
        if p.startswith("_"):
            test_parts.append(p)  # Keep underscore dirs as-is
        else:
            test_parts.append(f"test_{p}")

    # Prefix filename part (always prefixed)
    filename = parts[-1]
...
def _get_test_py_files() -> Set[Path]:
    ...
    return {p for p in TESTS_MIRROR_ROOT.rglob("test_*.py") if p.name not in ["__init__.py", "conftest.py"]}
...
def _expected_source_path(test_file: Path) -> Path:
    ...
    # Remove 'test_' prefix from directory parts ONLY if they start with it
    # Remove 'test_' prefix from filename (always)
    src_parts = []
    for p in relative_path.parts[:-1]:
        if p.startswith("test_"):
            src_parts.append(p.removeprefix("test_"))
        else:
            src_parts.append(p)  # Keep underscore dirs as-is

    # Remove 'test_' from filename
    filename = relative_path.parts[-1]
    src_filename = filename.removeprefix("test_")
    src_parts.append(src_filename)
...
def test_no_orphaned_test_files(test_file: Path):
    ...
    print(f"\nDEBUG: test_file = {test_file}")
    print(f"DEBUG: expected_source_file = {expected_source_file}")
    print(f"DEBUG: expected_source_file exists? = {expected_source_file.exists()}")
    print(f"DEBUG: expected_source_file is_file? = {expected_source_file.is_file()}")
    # --- END DEBUG ---
...
def test_no_test_prefix_in_source_files():
    ...
    violations = [p.relative_to(SRC_PKG_ROOT.parent) for p in SRC_PKG_ROOT.rglob("*.py") if p.name.startswith("test_")]
    assert not violations, "Found source files prefixed with 'test_' which is forbidden:\n - " + "\n - ".join(
        map(str, violations)
    )
...
def test_project_root_contains_only_whitelisted_items():
    ...
    try:
        ...
        allowed_items_config = (
            config.get("tool", {}).get("zeroth-law", {}).get("allowed-project-roots", {}).get("items", [])
        )
