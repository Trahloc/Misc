 ...
 *
 * 2024-04-26 Claude-3-Opus/Trahloc: GOOGLEMAPS_ELEVATION added
 *
 */
...
/**
 * Get the elevation of a location on Google Maps.
 *
 * =GOOGLEMAPS_ELEVATION("37.4219999","-122.0840575")
 *
 * @param {String} latitude The latitude of the location.
 * @param {String} longitude The longitude of the location.
 * @return {Number} The elevation in meters.
 * @customFunction
 */
const GOOGLEMAPS_ELEVATION = (latitude, longitude) => {
  ...
  const key = ["elevation", latitude, longitude].join(",");
  const value = getCache(key);
  if (value !== null) return value;

  const { results: [data = null] = [] } = Maps.newElevationSampler().sampleLocation(
    parseFloat(latitude),
