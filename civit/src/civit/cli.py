...
def setup_logging(
    verbosity_level: int = 0, quiet: bool = False, debug: bool = False
) -> None:
...
def parse_args(args: List[str] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "-o", "--output-folder", default=os.getcwd(), help="Output directory"
    )
    parser.add_argument(
        "--custom-naming",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="custom_naming",
        help="Do not use custom filename format",
    )
    parser.add_argument(
...
def main():
    ...
    if not args.api_key:
        ...
        if args.api_key:
            ...
            logger.debug(
                "No API key provided via argument or CIVITAPI environment variable"
            )
    else:
