...
from {{ cookiecutter.project_name }}.utils import get_project_root
...
def find_source_and_test_files(project_root: Path, project_name: str) -> Tuple[List[Path], List[Path]]:
    """
...
def calculate_coverage(source_files: List[Path], test_files: List[Path], project_name: str) -> Dict:
    """
    ...
    for file in test_files:
        ...
        for src_module in source_modules:
            ...
            if len(src_parts) > 1:  # Ignore the project name itself
                ...
                if test_file in rel_path and rel_path.replace("/", ".")[:-3].startswith(test_module.replace(f".{src_parts[-1]}", "")):
    ...
    coverage_percentage = 100.0 if total_source_modules == 0 else (covered_modules / total_source_modules) * 100
    ...
    if coverage_info['source_modules_without_tests']:
        ...
        if modules_without_tests > 0:
            ...
            if modules_without_tests <= 5:  # Only list if there are few missing tests
                ...
                click.echo("Run 'zeroth-law create-test-stubs' to generate test stubs for missing tests.")


def create_test_stubs(coverage_info: Dict, project_root: Path, project_name: str) -> None:
    """
...
''')
            ...
            click.echo(f"Test file already exists: {test_file.relative_to(project_root)}")
