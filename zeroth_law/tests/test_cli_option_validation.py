...
def test_cli_group_option_invocation(option):
    ...
    with patch.object(cli_module, "run_audit", return_value=False):
        ...
        assert result.exit_code in (0, 1, 2), f"CLI option {option} failed with exit code {result.exit_code}: {result.output}"
...
def test_audit_option_invocation(option, value):
    ...
    with patch.object(cli_module, "run_audit", return_value=False):
        ...
        assert result.exit_code in (0, 1, 2), f"Audit command option {option} failed with exit code {result.exit_code}: {result.output}"
...
def test_install_hook_option_invocation(option, value):
    ...
    with (
        ...
        result = runner.invoke(cli_group, args)
        assert result.exit_code in (0, 1), f"Install hook command option {option} failed with exit code {result.exit_code}: {result.output}"
...
def test_restore_hooks_option_invocation(option, value):
    ...
    with (
        patch.object(Path, "is_dir", return_value=True),
        patch("subprocess.run", return_value=type("obj", (object,), {"stdout": "", "stderr": ""})),
    ):
        # Build command arguments
